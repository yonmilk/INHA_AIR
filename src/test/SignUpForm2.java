...
public class SignUpForm2 extends JFrame implements ActionListener{
	...
	private JTextField tfID, tfEmail;
	
	private JFormattedTextField tfBirth, tfPhone, tfPassport, tfLNEng, tfLNKor;
	...
	private Component lblPassportEx;
	...
	private void setSignUpForm() {
		...
		try {
			//MaskFormatter format = new MaskFormatter("?????????????????");	//우리나라 이름 17자까지
			//tfLNKor = new JFormattedTextField(format);
			tfLNKor = new JFormattedTextField();
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			//MaskFormatter format = new MaskFormatter("UUUUUUUUUUUUUUUUUUUUUUUUUUUUUU");
			//tfLNEng = new JFormattedTextField(format);
			tfLNEng = new JFormattedTextField();
		} catch (Exception e) {
			e.printStackTrace();
		}
